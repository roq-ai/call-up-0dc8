...
import { getCallInitiatorById, updateCallInitiatorById } from 'apiSdk/call-initiators';
...
function CallInitiatorEditPage() {
  ...
  const handleSubmit = async (values: CallInitiatorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCallInitiatorById(id, values);
      mutate(updated);
      resetForm();
