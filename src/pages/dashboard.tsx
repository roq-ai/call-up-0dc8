...
import { getCallInitiators } from 'apiSdk/call-initiators';
import { CallInitiatorInterface } from 'interfaces/call-initiator';
import { getSiteAdministrators } from 'apiSdk/site-administrators';
import { SiteAdministratorInterface } from 'interfaces/site-administrator';

import { CallReceiverListPage as ShowCaseList } from 'pages/call-receivers';
...
function DashboardPage() {
  ...
  const callInitiatorStat = useSWR<PaginatedInterface<CallInitiatorInterface>>(
    () => `/call-initiators`,
    () => getCallInitiators({ limit: 1, offset: 0 }),
  );

  const siteAdministratorStat = useSWR<PaginatedInterface<SiteAdministratorInterface>>(
    () => `/site-administrators`,
    () => getSiteAdministrators({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Call Initiators',
      result: callInitiatorStat?.data?.totalCount,
      isLoading: callInitiatorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Site Administrators',
      result: siteAdministratorStat?.data?.totalCount,
      isLoading: siteAdministratorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Call Receivers
                </Text>
                <NextLink href={`call-receivers`} passHref legacyBehavior>
                  <Button
