...
import { getCallReceiverById, updateCallReceiverById } from 'apiSdk/call-receivers';
...
function CallReceiverEditPage() {
  ...
  const handleSubmit = async (values: CallReceiverInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCallReceiverById(id, values);
      mutate(updated);
      resetForm();
